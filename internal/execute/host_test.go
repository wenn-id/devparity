...
func TestRunHostSuccessUsesMinimalForwardedEnvironment(t *testing.T) {
	...
	block := model.DocBlock{ID: "README.md:2", Shell: "sh", Script: shellScript(`printf '%s|%s|%s' "$DEVPARITY_TEST_SECRET" "${DEVPARITY_TEST_ABSENT-}" "${DEVPARITY_NOT_FORWARDED-}"`, `$env:DEVPARITY_TEST_SECRET; Write-Output "$env:DEVPARITY_TEST_SECRET|$env:DEVPARITY_TEST_ABSENT|$env:DEVPARITY_NOT_FORWARDED"`)}
...
func TestRunHostNonzeroExit(t *testing.T) {
	...
	result, err := RunHost(context.Background(), grant, model.DocBlock{Shell: "sh", Script: shellScript("exit 7", "exit 7")}, Options{Root: t.TempDir(), Timeout: time.Second})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunHostTimeout(t *testing.T) {
	...
	result, err := RunHost(context.Background(), grant, model.DocBlock{Shell: "sh", Script: shellScript("sleep 1", "Start-Sleep -Seconds 1")}, Options{Root: t.TempDir(), Timeout: 100 * time.Millisecond})
	if err != nil {
		t.Fatal(err)
	}
